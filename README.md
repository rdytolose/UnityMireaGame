...
| Адрес сервера в **готовом билде** | `server.cfg` рядом с билдом | поменять сервер без пересборки (см. ниже). |
| Адрес сервера в **исходниках** | `scripts/Backend/BackendConfig.cs` → `DefaultBaseUrl` | значение по умолчанию, если `server.cfg` нет. |

> Правило простое: **адрес и порт в `.env` и в игре должны совпадать.**
> Для теста пейринга с телефона в локалке поставь IP машины в сети, напр.
> `http://192.168.0.10:8081` (а не `localhost`).

### Указать свой сервер в готовом билде (без пересборки)

Игра при старте читает текстовый файл **`server.cfg`** и берёт адрес сервера оттуда.
Шаблон уже едет в комплекте, в нём один параметр:

```ini
server_url=http://my-server.com:8081
```

Куда положить (первый найденный побеждает):
1. **рядом с исполняемым файлом** игры (`server.cfg` в одной папке с `.exe`) — удобнее всего;
2. либо отредактировать тот, что внутри билда: `<Игра>_Data/StreamingAssets/server.cfg`.

Если файла нет или адрес пустой — берётся `DefaultBaseUrl` из исходников.
Открывается любым текстовым редактором (Блокнот). После правки — просто перезапусти игру.
...
Workflow `.github/workflows/deploy.yml` при пуше в `main` работает «по изменениям»:
1. **changes** — смотрит, что поменялось: `temp/backend/**`, `temp/frontend/**`,
   `temp/docker-compose.yml`;
2. **build-backend / build-frontend** — собирают и пушат в **GHCR**
   (`ghcr.io/rdytolose/unitymireagame-backend|frontend`) **только изменившийся** образ;
3. **deploy** — по SSH на сервере (`git reset --hard` → `docker compose pull` →
   `up -d`), запускается только если что-то собралось или менялся compose. Сервер
   только тянет готовые образы, ничего не компилирует.

Так пуш, который трогает только README/клиент Unity, **не** запускает сборку и деплой.
Ручной запуск (`Run workflow`) пересобирает и деплоит всё принудительно.
